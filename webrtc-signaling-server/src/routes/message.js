...
router.post('/upload', upload.single('file'), (req, res) => {
  ...
  // Create proper file URL using environment variable or request origin
  const baseUrl = process.env.NODE_ENV === 'production' 
    ? (process.env.BACKEND_URL || req.get('origin')?.replace('https://webrtc-client', 'https://webrtc-signaling-server') || 'https://webrtc-signaling-server.onrender.com')
    : 'http://localhost:5000';
